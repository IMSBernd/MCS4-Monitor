...
class MainWindow(QMainWindow):
    ...
    def apply_language(self) -> None:
        ...
        self.tabs.setTabText(8, self.tr("tabs_sensor_configuration"))
        self.tabs.setTabText(9, self.tr("tabs_alarms"))
        self.tabs.setTabText(10, self.tr("tabs_export"))
        self.tabs.setTabText(11, self.tr("tabs_diagnostics"))
    ...
    def _build_ui(self):
        ...
        self.compliance_table = QTableWidget(0, 5)
        self.compliance_table.setHorizontalHeaderLabels(self.translations.get("compliance_headers", []))
    ...
    def _compliance_rows(self):
        ...
        open_item = self.tr("compliance_status_open")
        ok = self.tr("compliance_next_ok")
        rows = [
            ("RS422 serial format", "38400 Baud, 8 data bits, odd parity, 1 stop bit", "Configured in RS422 start routine", implemented, ok),
            ("Byte 1 Sync", "Byte 1 must be FF", "Frame reader synchronizes on 0xFF", implemented, ok),
            ("Byte 2 WordType", "D3..D0 = WordType", "decode_word_type(header & 0x0F)", implemented, ok),
            ("Byte 2 Flags", "D7..D4 = word-specific extension bits", "decode_word_flags(header >> 4); shown in explorer", implemented, ok),
            ("WordType lengths", "WT 0/1/6/7 = 8 bytes, WT 2/3/4/5 = 5 bytes", "WORD_LENGTHS table used by frame reader", implemented, ok),
            ("Undefined WordTypes", "WT 8..14 are not defined/reserved", "Rejected/resynchronized, not converted to sensor values", implemented, ok),
            ("Data Value only to dashboard", "Only WT 0 carries live analog data values", "Only WT 0 updates dashboard/trend", implemented, ok),
            ("Byte 3 Target", "Target device/group address 0..127", "Validated and shown in explorer", implemented, ok),
            ("Byte 4 Source", "Sender device address 0..63", "Validated and shown in explorer", implemented, ok),
            ("Byte 5 Measuring Point", "Measuring point number; not a fixed sensor name", "Used with Page/Line as composite key", implemented, ok),
            ("Byte 6 Page/Line", "D7..D3 = Page, D2..D0 = Line", "decode_page_line(byte6)", implemented, ok),
            ("Data Value Byte 7/8", "D6 sensor fault, D5 sign, D4..D0 + Byte8 D6..D0 = 12-bit value", "decode_12bit_value(byte7, byte8)", implemented, ok),
            ("Sensor fault", "Byte7 D6 marks sensor fault", "Evaluated and passed to alarm/status logic", implemented, ok),
            ("Sign bit", "Byte7 D5 marks negative value", "Applied before scaling", implemented, ok),
            ("Appendix 12 scaling", "Page/Line selects unit, range and factor", "SCALING_TABLE present but still needs final full audit against all appendix rows", partial, self.tr("compliance_next_appendix12")),
            ("WordType 1 Limit Value", "Separate WT 1 structure, static/dynamic and header/value flags", "Classified and displayed; no full limit decoder yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 2 Alarm", "5-byte alarm message with alarm-specific flags", "Classified and counted; not fully decoded yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 3 Control Command", "5-byte control command", "Classified and counted; not used as sensor value", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 4 Binary Signal", "5-byte binary state message", "Classified and counted; not fully decoded yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 5 Key ID", "5-byte key identification", "Classified and counted; not fully decoded yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 6 Status Message", "8-byte status message", "Classified and counted; not fully decoded yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("WordType 7 Curve Transfer", "8-byte curve transfer", "Classified and counted; not fully decoded yet", partial, self.tr("compliance_next_wordtype_decoder")),
            ("Checksum / CRC", "Only where specified by individual word headers", "Displayed where present; no universal CRC applied", partial, self.tr("compliance_next_crc")),
            ("Regression tests", "Decoder should be checked against simulator and real mcslog files", "Manual testing only so far", open_item, self.tr("compliance_next_regression")),
        ]
        return rows
    ...
    def populate_compliance_table(self):
        ...
        for row, values in enumerate(rows):
            for col, text in enumerate(values):
                ...
                if col == 3:
                    if text == self.tr("compliance_status_implemented"):

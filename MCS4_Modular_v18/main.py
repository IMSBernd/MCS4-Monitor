...
def default_sensor_config() -> dict:
    return {
        "sensors": [
            {"id": 1, "name": "Öltemperatur", "page": 3, "line": 0, "unit": "°C", "warn_low": 0, "warn_high": 90, "alarm_low": -10, "alarm_high": 95},
            {"id": 2, "name": "Öldruck", "page": 1, "line": 1, "unit": "bar", "warn_low": 4.5, "warn_high": 6.0, "alarm_low": 4.0, "alarm_high": 6.5},
            {"id": 3, "name": "Drehzahl", "page": 8, "line": 0, "unit": "rpm", "warn_low": 500, "warn_high": 1900, "alarm_low": 300, "alarm_high": 2100},
            {"id": 4, "name": "Kühlwasser", "page": 3, "line": 0, "unit": "°C", "warn_low": 0, "warn_high": 88, "alarm_low": -10, "alarm_high": 95},
            {"id": 5, "name": "Abgastemperatur", "page": 4, "line": 0, "unit": "°C", "warn_low": 0, "warn_high": 520, "alarm_low": -10, "alarm_high": 560}
        ]
    }
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.config_table.setHorizontalHeaderLabels(["ID", "Name", "Page", "Line", "Einheit", "Warn Low", "Warn High", "Alarm Low", "Alarm High"])
    ...
    def load_sensor_config_from_file(self):
        ...
        for entry in self.config_data.get("sensors", []):
            ...
            self.sensor_unit_codes[sid] = int(entry.get("unit_code", 0) or 0)
            self.limits[sid] = {
    ...
    def populate_config_table(self):
        ...
        sensors.sort(key=lambda e: int(e.get("id", 0)))
        self.config_table.setRowCount(len(sensors))
        columns = ["id", "name", "page", "line", "unit", "warn_low", "warn_high", "alarm_low", "alarm_high"]
    ...
    def save_sensor_config_from_table(self):
        ...
        for row in range(self.config_table.rowCount()):
            try:
                ...
                entry = {
                    "id": int(cell(0)),
                    "name": cell(1) or f"Messpunkt {cell(0)}",
                    "page": int(cell(2)) if cell(2) != "" else "",
                    "line": int(cell(3)) if cell(3) != "" else "",
                    "unit": cell(4),
                    "warn_low": float(cell(5).replace(",", ".")) if cell(5) != "" else 0.0,
                    "warn_high": float(cell(6).replace(",", ".")) if cell(6) != "" else 0.0,
                    "alarm_low": float(cell(7).replace(",", ".")) if cell(7) != "" else 0.0,
                    "alarm_high": float(cell(8).replace(",", ".")) if cell(8) != "" else 0.0,
                }
    ...
    def add_detected_sensor_to_config(self, measuring_point: int, page: int, line: int, unit: str) -> None:
        ...
        """
        if measuring_point in self.sensor_names:
            return
        ...
        entry = {
            "id": measuring_point,
            "name": f"Messpunkt {measuring_point}",
            "page": page,
            "line": line,
            "unit": unit,
            "warn_low": 0.0,
        ...
        self.config_data["sensors"].sort(key=lambda e: int(e.get("id", 0)))
        CONFIG_FILE.write_text(json.dumps(self.config_data, indent=2, ensure_ascii=False), encoding="utf-8")
        self.load_sensor_config_from_file()
        self.populate_config_table()
        self.alarm_log.appendPlainText(
            f"{datetime.now():%H:%M:%S} [INFO] Neuer Messpunkt erkannt: {measuring_point} (Page {page}, Line {line}, {unit or 'ohne Einheit'})"
        )
    ...
    def decode_packet(self, packet: bytes):
        ...
        name = self.sensor_names.get(measuring_point, f"Messpunkt {measuring_point}")
        ...
        state, alarm_text = self.evaluate_sensor(measuring_point, name, value, unit, sensor_fault)

        old = self.sensor_values.get(measuring_point)
        min_value = value if old is None else min(old["min"], value)
        max_value = value if old is None else max(old["max"], value)

        self.sensor_values[measuring_point] = {
            "id": measuring_point,
            "name": name,
            "value": value,
            "unit": unit,
        ...
        self.update_trend(measuring_point, value, name)
        self.record_packet(packet)

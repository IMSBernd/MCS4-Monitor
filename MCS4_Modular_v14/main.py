...
class MainWindow(QMainWindow):
    ...
    def export_excel(self):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        path = EXPORT_DIR / f"sensor_snapshot_{timestamp}.xlsx"
        wb = Workbook()
        ws = wb.active
        ws.title = "Sensoren"
        headers = ["ID", "Sensor", "Wert", "Einheit", "Status", "Alarm", "Min", "Max", "Zeit"]
        ws.append(headers)
        for cell in ws[1]:
        ...
        wb.save(path)
        self.export_log.appendPlainText(f"{datetime.now():%H:%M:%S} Excel exportiert: {path}")
